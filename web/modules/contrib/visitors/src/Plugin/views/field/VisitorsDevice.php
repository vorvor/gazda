 ...
 *
 * @ViewsField("visitors_device")
 */
final class VisitorsDevice extends FieldPluginBase {
  ...
  public function render(ResultRow $values) {
    $output = '';
    $device_type = $this->getValue($values) ?? 'unknown';

    $image = $this->path . '/icons/devices/' . $device_type . '.png';
    if ($this->options['icon']) {
      $output .= '<img src="' . $image . '" width="16" height="16" /> ';
    }

    $output .= ucwords($device_type);

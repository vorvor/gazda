...
use Drupal\visitors\Plugin\views\field\VisitorsOperatingSystem;
 ...
 *
 * @group visitors
 * @coversDefaultClass \Drupal\visitors\Plugin\views\field\VisitorsOperatingSystem
 * @uses \Drupal\visitors\Plugin\views\field\VisitorsOperatingSystem
 */
class VisitorsOperatingSystemTest extends UnitTestCase {

  /**
   * The field.
   *
   * @var \Drupal\visitors\Plugin\views\field\VisitorsOperatingSystem
   */
  protected $field;
  ...
  protected function setUp(): void {
    ...
    $plugin_definition = [];
    $this->field = VisitorsOperatingSystem::create($container, $configuration, $plugin_id, $plugin_definition);
    $options = [];
    $options['id'] = 'id';
    $options['icon'] = TRUE;
  ...
  public function testCreate() {
    ...
    $plugin_id = 'visitors_brand';
    $plugin_definition = [];
    $field = VisitorsOperatingSystem::create($container, $configuration, $plugin_id, $plugin_definition);
    $this->assertInstanceOf('Drupal\visitors\Plugin\views\field\VisitorsOperatingSystem', $field);
  }
  ...
  public function testConstruct() {
    $field = new VisitorsOperatingSystem([], 'visitors_brand', [], $this->moduleHandler, $this->request);
    $this->assertInstanceOf('Drupal\visitors\Plugin\views\field\VisitorsOperatingSystem', $field);
  }
  ...
  public function testRender() {
    ...
    $values = new ResultRow(['alias' => 'MAC']);
    $this->field->field_alias = 'alias';

    $this->assertEquals('<img src="/base/path/modules/contrib/visitors/icons/os/MAC.png" width="16" height="16" /> Mac', (string) $this->field->render($values));
  }
  ...
  public function testRenderUnknown() {
    $this->field->options['icon'] = FALSE;

    $values = new ResultRow(['alias' => '']);
    $this->field->field_alias = 'alias';

    $this->assertEquals('', (string) $this->field->render($values));
  }

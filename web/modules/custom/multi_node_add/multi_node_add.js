...
(function ($, Drupal, drupalSettings) {
  ...
  function multiNodeAddCheckConfig(numForms) {
    if (!('multiNodeAddPreload' in drupalSettings)) {
      if ($("input.multi-node-add:checked").length == 0) {
  ...
  function multiNodeAddShowForms(numForms) {
    ...
    for (var i = 0; i < numForms; i++) {
      var fields = '';
      if ('multiNodeAddPreload' in drupalSettings) {
        fields = drupalSettings.multiNodeAddPreload.fields.join(',');
      }
      else {
  ...
  Drupal.behaviors.multiNodeAdd = {
    attach: function (context) {
        ...
        $('#edit-show', context).click(function () {
          try {
            if (multiNodeAddShowForms($("#edit-number").val())) {
              ...
              $('input.form-checkbox.multi-node-add').prop('disabled', true);
            }
          }
          catch (err) {
            alert(err);
      ...
      $('#edit-prepopulate', context).click(function() {
        ...
        if (multiNodeAddNumNodes < 2 || $('input[type=submit]', window.frames['node_0'].document).length == 0) {
        ...
        $('input:not([type=hidden]):not([type=submit]):not([type=button]):visible,' +
          'select:visible,' +
          'textarea:visible', window.frames['node_0'].document).each(function() {

            for (var i = 1; i < multiNodeAddNumNodes; i++) {
              try {
                if ($(this).is(':radio, :checkbox')) {
                  var checked = $(this).is(':checked');
                  $('#' + $(this).attr('id'), window.frames['node_' + i].document).prop('checked', checked);
                } else {
                  $('#' + $(this).attr('id'), window.frames['node_' + i].document).val($(this).val());
                }
              } catch (err) {} // not an error, maybe submitted w/ the single Create button
            }
